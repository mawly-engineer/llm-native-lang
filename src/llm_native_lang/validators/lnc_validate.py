...
def validate_file(path: str, seen_ids: Set[str] = None) -> Tuple[bool, List[str]]:
    """Public API: Validate a single LNC file."""
    if seen_ids is None:
        seen_ids = set()
    return _validate_file(path, seen_ids)


def validate_directory(dirpath: str, seen_ids: Set[str] = None) -> Tuple[bool, List[Tuple[str, str]]]:
    """Public API: Validate all LNC files in a directory."""
    if seen_ids is None:
        seen_ids = set()
    
    failures = []
    targets = list(_iter_targets(dirpath))
...
def main(argv=None) -> int:
    """Main entry point for LNC validation."""
    if argv is None:
        argv = sys.argv[1:]
