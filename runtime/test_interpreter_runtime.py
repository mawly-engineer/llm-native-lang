...
class InterpreterRuntimeLexicalScopeTest(unittest.TestCase):
    ...
    def test_object_string_index_access_returns_member(self) -> None:
    ...
    def test_object_string_index_access_missing_field_raises_structured_error(self) -> None:
        with self.assertRaises(EvalError) as ctx:
            eval_expr(parse_expr('user["missing"]'), env={"user": {"name": "Luis"}})
        self.assertEqual(ctx.exception.code, "E_RT_MISSING_MEMBER")

    def test_object_string_index_access_rejects_non_string_index(self) -> None:
        with self.assertRaises(EvalError) as ctx:
