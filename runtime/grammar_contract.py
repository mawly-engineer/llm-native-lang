...
string literals, plus/minus operators (+/-) for deterministic string concat/int arithmetic, null-coalescing (??), equality/comparison tiers, exponentiation (**), multiplication (*), exact division (/), modulo (%), integer division (//), logical and/or, bool literals,
...
class _Parser:
    ...
    def _multiplicative(self) -> dict[str, Any]:
        ...
        while self._peek().kind in {"*", "/", "%", "//"}:
            ...
            right = self._power()
            node = self._with_span(
                "mul_bin" if op == "*" else ("exact_div_bin" if op == "/" else ("modulo_bin" if op == "%" else "int_div_bin")),
                node["span"]["start"],
                right["span"]["end"],
                left=node,
                right=right,
